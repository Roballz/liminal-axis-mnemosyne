...
适用分支：`work/daily-memory-mvp-baibai`，2026-09-24 编辑审核与多表补表修订版。状态仍为 `implemented_unverified`；本轮运行受影响 Node 测试、类型检查与构建，未重跑浏览器或实际 TT 手机测试。
...
| 保留摘要 | 人工确认该摘要对当前正文版本仍适用 | 不调用模型，仅对已确认的来源和依赖版本生效；无关后续追加不撤销确认 |
...
事件保存在本机独立 IndexedDB `mnemosyne_daily_*` 中，按正式故事和分支隔离。父分支与子分支即使消息条数、文字完全一样，仍是两个分支，不能根据文字自动合并。事件页跟随当前聊天重新读取。当前档案的事件在来源待审核时仍显示原概要和关联，标记待更新；历史截止视图仍严格过滤。待确认概要不自动进入召回。
...
旧 L0 摘要若没有精确输入记录，仍按声明的来源前缀检查，不猜测来源；有明确下级依赖的高层摘要按依赖检查。当前事件页保留待确认概要和关联。“保留摘要”记录你对当前来源版本的确认，摘要版本未变时相关事件可随之恢复；摘要版本变化后，事件概要可独立保留或更新。若需要反馈，可只提供事件页的已保存数量、来源差异类型与楼层，正文片段可遮盖，无需上传整个私人核心包。
...
新版导出 `mnemosyne-daily-v6.json`，包含事件归档标记、人工确认的隐藏来源修复记录，以及接回档案后保留的未连接绑定，支持恢复旧 v1 / v2 / v3 / v4 / v5 包。物理 IndexedDB 仍为版本 1，不升级 object store、不删旧库。旧版扩展不能理解新的归档标记及其他新增字段，回退时保留新版包，使用更新前的备份恢复旧版。
...
表列表页点“批量补表”，多选表后，每张表显示最后确认楼号 → 当前最新楼号 / 总楼数，统一填写所有选中表的共用起止楼号（从0开始）。没有字段的空表先设计字段。最后确认楼号不是“之前每楼全部已处理”的保证，可回头选择缺失范围。

确认后用摘要 API 发送**正文而非摘要**，附全部选中表的定义、字段填写提示词和当前可见行。每批楼数可输入设置，同一批只发送一次正文并填多张表；每次返回先进入编辑窗口，确认时再次校验，通过才原子保存全部表及正文版本回执。明确无变化也需确认后记进度。关闭“随摘要填写”的表仍可主动补表，原自动开关不改变。
...
正文编辑自动归档为新版本。未被摘要引用的末楼编辑、删除和重新回复，不再令仅依赖前面摘要的高层节点整批待审核。实际受影响摘要在摘要页、事件页和档案页提供同一审核入口：保留全部现有摘要、逐条保留、手动修订、按依赖顺序更新受影响摘要。只有明确选择模型更新才由这个审核入口调用 API；停止或失败保留已完成部分。

“保留”适合错字或不影响总结含义的改动。兼容确认按实际来源和所选依赖版本保存，无关追加不撤销确认，再次修改相关来源仍重新审核。来源删除、身份重排或依赖缺失不能直接保留。原始摘要的来源证据不改写。

手改摘要保存新版本，事件成员按原摘要身份跟随当前版本；旧事件和关联不会因编辑消失。事件页可保留现有事件概要、手动更新并确认，或使用模型更新／重写概要后审阅返回。保留摘要且其内容版本未变，不要求另重写事件；摘要文字改变后，可以独立判断事件概要是否需要改变。待审核、已删除或禁用的成员先处理来源，不能通过保留概要绕过。

自定义表内容和字段保存在本机 Mnemosyne IndexedDB，核心包包含它们。改表不会产生摘要／事件审核；表定义或内容改变会拒绝此前尚未确认的旧补表返回。

批量补表使用一个共用起止楼号，多选表在每批一起发送，正文只发送一次；“每批楼数”可输入正整数并记住设置。每批返回无论是否通过校验，都进入编辑窗口，保留接口实际返回正文（含空返回或截断 JSON）。手改后确认会再次检验全部所选表，通过才整批写入，不再次调用模型；失败留在窗口继续修改，取消停止后续批次，已经确认的批次保留。网络请求失败且未取得正文时仍显示请求错误。

核心包现在导出 v6，继续读取 v1～v5。新增版本化审核字段和每批楼数设置；旧版本扩展不能读取含这些字段的 v6 包。需要回退时保留旧核心包，旧包之后的新修改不包含在其中；不要清库或覆盖旧备份。
