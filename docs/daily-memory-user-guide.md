...
- **从所选聊天分叉（原聊天保留）**：适合 TT 的“创建分支”。例如原聊天有100条消息，在第60条之后另开支线，就继承开头60条；此后两条路线独立发展。原聊天剩余消息不删除、不覆盖。分叉不复制父分支的事件链或自定义表。
- **沿用所选分支继续（不新建支线）**：用于同一路线更换聊天文件，沿用同一归档分支身份；不是登记一条独立支线。
