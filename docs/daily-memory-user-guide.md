...
1. **新建事件链**：输入整体事项说明，例如“把这一天的早餐、做花灯、逛街和灯会作为一次完整约会”。点击新建后还有一次 API 确认。程序发送原生单楼摘要使用的角色卡、用户设定、世界书、此前状态/摘要等材料，加本楼正文、摘要和你的说明，仅调用一次摘要 API，生成链名、初始概要、关键词并关联本楼；不重写本楼摘要或人物/物品。
2. **加入已有链**：选链并确认，再选“立即更新”或“仅入库”。后者不调用模型。前者先确保本楼已入链，再发该链全部有效成员材料，覆盖更新当前概要、关键词，并追加本次新进展；已有本楼不会重复关联。全部成员已更新时不再花费一次请求。

可连续多楼选择“仅入库”，等事件收束再更新一次。新建的边界说明进入本次初始标题/概要；后续按这条链的标题、概要及全部成员继续整理，模型不再有拆链或新建链权限。番外、没有摘要或摘要待审核的楼需先处理后再入链。

楼层事件操作的成功、失败、请求超限等结果以弹窗提示，点击“确定”关闭；卡片按钮下不留提示文字。失败时保留输入，可关闭提示后重试。若本楼已入库但概要更新失败，弹窗会明确说明；这时关联仍保留，概要继续待更新。

新生成或手动编辑的事件概要不超过 **500字（含标点、数字、字母，去掉首尾空白后计数）**。AI 只保留事件核心变化、关键决定与结果，以及对关系或重要转变有实质影响的细节，舍弃琐碎流水账。超长模型结果会被拒绝，不截断保存、不自动增加一次 API 调用，也不推进已整理范围。编辑弹窗显示字数，超过上限不能保存。旧概要不会自动改写；可手动精简，或有新成员待处理时通过下次更新生成短概要。

**自定义概括方式**：进入“设置 → 自定义提示词 → 事件概要提示词”。与摘要、二次总结使用同一编辑弹窗，默认预填现用规则，可直接修改重点、取舍和写法；点击“完成”才保存，“取消”放弃草稿。“恢复默认”后再点“完成”恢复内置规则，留空也使用默认。保存后从下一次新建事件、立即更新、批量整理或自动更新请求生效；不会自动重写已保存概要。
