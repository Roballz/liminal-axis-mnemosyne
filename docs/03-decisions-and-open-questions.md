...
07 保留为首轮候选；冲突的依赖描述由本节和 08 取代。Head 当前以 08 的确认版本为准。T-02 最小契约/参考模型已实施，保持 implemented_unverified 待 review；T-03 为预备 proposal。上述 accepted 规则未由实现静默改变。

## 11. 任务就绪与剩余事项（2026-09-19）

- **T-02：implemented_unverified。** `tasks/T-02-identity-version-context.md`。已交最小可执行契约/校验、内存参考模型与正反例，不含正式数据库/导入器/LLM 重建器。原24项对应关系、安全未决状态和实测证据列于卡内，需 Chat review 后才能 verified。
