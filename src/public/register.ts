...
export const PUBLIC_READY_EVENT = 'mnemosyne:ready';
export const PUBLIC_CHANGED_EVENT = 'mnemosyne:changed';
...
async function registerSlashCommand(): Promise<boolean> {
  try {
    ...
    console.warn('[柏宝书] /mnemosyne-get 注册失败', error);
    return false;
  }
}
...
async function registerMacros(): Promise<{ macros: boolean; parameterized: boolean }> {
  try {
    ...
    if (powerUserModule.power_user?.experimental_macro_engine) {
      ...
      const definitions: Array<[string, Record<string, unknown>]> = [
        ...
        ['mnemosyneSnapshot', {
          ...
          exampleUsage: ['{{mnemosyneSnapshot}}', '{{mnemosyneSnapshot::42::after}}'],
        ...
        ['mnemosyneHistory', {
          ...
          exampleUsage: ['{{mnemosyneHistory}}', '{{mnemosyneHistory::42}}'],
        ...
        ['mnemosyneInjectedHistory', {
          ...
          exampleUsage: ['{{mnemosyneInjectedHistory}}'],
        ...
        ['mnemosyneVar', {
          ...
          exampleUsage: ['{{mnemosyneVar::关系.爱丽丝.好感度}}', '{{mnemosyneVar::队伍[0].hp::42::after}}'],
        ...
        ['mnemosyneFloor', {
          ...
          exampleUsage: ['{{mnemosyneFloor::42}}'],
    ...
    const stableMacros: Array<[string, () => string, string]> = [
      ['mnemosyneVars', () => macroSafe(() => getSnapshot().vars), '返回柏宝书当前自定义变量 JSON。'],
      ['mnemosyneState', () => macroSafe(() => getSnapshot().state), '返回柏宝书当前状态 JSON。'],
      ['mnemosyneSnapshot', () => macroSafe(() => getSnapshot()), '返回柏宝书当前完整状态快照 JSON。'],
      ['mnemosyneHistory', () => macroSafe(() => getHistory().relativeText), '返回柏宝书带相对时间的压缩历史剧情文本。'],
      ['mnemosyneInjectedHistory', () => macroSafe(() => getInjectedHistory().relativeText), '返回与正常记忆注入相同、已跳过滑动窗口的历史剧情文本。'],
    ];
...
export async function registerPublicInterface(): Promise<void> {
  ...
  bumpPublicRevision();
  const api = createApi();
  (globalThis as typeof globalThis & { STMnemosyne?: STBaiBaiBookApi }).STMnemosyne = api;
