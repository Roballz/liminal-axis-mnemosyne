...
import { capture, synchronize, statuses, current, forkBranch, type HostObservation, type HostMemory } from './canonical';
import { row, check, type CapturedView, type SourceRef, type Branch, type Binding } from './model';
export const dailyState = reactive({ status: '未归档', error: '', pending: false, generation: 0,
    branch: '', archived: 0, summaries: 0, review: 0, conflict: false, revision: 0 });
let installed = false;
let cache: {
    view: CapturedView;
    allowed: Set<string>;
...
export function hostVersion(): string {
    const ctx = getContext();
    return JSON.stringify([hostScope(), (ctx?.chat ?? []).map(m => [m.extra?.[MESSAGE_KEY], m.is_user, m.mes, m.swipe_id,
            m.extra?.bbs_omit, m.extra?.bbs_leaf]), memory.summaries]);
}
...
export function dailyInstalled() { return installed; }
export function summaryAllowed(hostId: string): boolean {
    return !installed || (!!cache && cache.host === hostVersion() && cache.allowed.has(hostId));
}
...
export async function syncDaily(): Promise<CapturedView> {
    const job = serial.catch(() => { }).then(async () => {
        ...
        const observation: HostObservation = { scope, messages: ctx.chat.map(m => ({ key: String(m.extra![MESSAGE_KEY]),
                role: m.is_user ? 'user' : ((m.extra?.bbs_internal_notice || (m.is_system && !m.extra?.bbs_hidden)) ? 'system' : 'assistant'), content: m.mes, swipe: m.swipe_id ?? 0 })), memories: [] };
        ctx.chat.forEach((m, i) => {
            ...
            const item: HostMemory = { hostId: leaf.id, content: leaf.text, level: 0, anchorKey: String(m.extra![MESSAGE_KEY]),
                children: [], storyTime: [leaf.timeStart, leaf.timeEnd].filter(Boolean).join(' - ') || leaf.timeLabel || '',
                seed: !!leaf.seed, enabled: leafValid(m) && !m.extra?.bbs_omit };
            if (evidence?.leaf === leaf.id && evidence.text === leaf.text) {
                ...
                item.declaration = '来源记录包含实际正文输入；角色卡、世界书及派生状态另作为非正文上下文，不等同已发生剧情。';
            }
            observation.memories.push(item);
        });
        for (const s of [...memory.summaries].sort((a, b) => a.level - b.level))
            observation.memories.push({
                hostId: s.id, content: s.text, level: s.level, anchorKey: null, children: s.childIds, generationKey:s.createdAt, storyTime: s.timeStart || s.timeLabel || '', seed: false, enabled: true
            });
        // Host writes happen first. If canonical fails, these IDs remain retryable; there is no cross-store atomicity claim.
        if (dirty || cache?.host !== host)
            ctx.chatMetadata.mnemosyne_ids_pending = true;
        if (ctx.chatMetadata.mnemosyne_ids_pending) {
            await ctx.saveChat();
            delete ctx.chatMetadata.mnemosyne_ids_pending;
        }
        check(scope === hostScope() && generation === dailyState.generation && host === hostVersion(), '聊天在归档期间改变，待重试');
        ...
        check(scope === hostScope() && generation === dailyState.generation && host === hostVersion(), '归档已保存；界面已变化，请重试当前聊天');
        ...
        const leaves: CanonicalLeaf[] = view.memories.filter(m => m.level === 0 && !m.seed && validity.get(m.id) === 'valid').map(m => {
            const idx = view.refs.findIndex(r => r.message === m.anchor);
            const source = view.sources.get(view.refs[idx]?.revision);
            return { leafId: m.id, hostId: m.hostId, docHash: m.fingerprint, payloadHash: source?.fingerprint || m.fingerprint,
                document: m.content, mesFull: source?.content ?? '', storyTime: m.storyTime, msgIndex: idx };
        });
        cache = { view, allowed, leaves, host };
        Object.assign(dailyState, { branch: branch.id, archived: view.refs.length, summaries: view.memories.length,
            review: [...validity.values()].filter(v => v === 'needs_review' || v === 'needs_rebuild').length,
            pending: false, status: '已归档', error: '', revision: dailyState.revision + 1 });
        if (installed)
            refreshInjection();
        return view;
    });
    serial = job;
    return job.catch(error => { dailyState.pending = true; dailyState.status = 'pending · 可重试'; dailyState.error = String(error.message ?? error); cache = null; throw error; });
}
export function scheduleDaily() {
    ...
    if (timer)
        clearTimeout(timer);
    timer = setTimeout(() => { void syncDaily().catch(() => { }); }, 200);
}
...
export async function dailyCurrent(view: CapturedView, host: string, generation: number) {
    return host === hostVersion() && generation === dailyState.generation && await current(await activeLibrary(), view);
}
...
export async function confirmBinding(branchId: string) {
    ...
    await lib.transaction(['host_bindings', 'branches'], 'readwrite', async (tx) => {
        ...
        check(ctx.chat.every(m => typeof m.extra?.[MESSAGE_KEY] === 'string'), '缺少原消息身份，不能直接续接');
        binding.scope = scope;
        binding.generation++;
...
export function bindDaily() {
    ...
    for (const key of ['CHAT_CHANGED', 'MESSAGE_EDITED', 'MESSAGE_DELETED', 'MESSAGE_SWIPED', 'MESSAGE_SENT', 'CHARACTER_MESSAGE_RENDERED', 'GENERATION_ENDED']) {
    ...
    scheduleDaily();
    return () => { installed = false; unwatch(); if (timer)
        clearTimeout(timer); for (const h of handlers)
        ctx.eventSource.off?.(h.event, h.callback); cache = null; };
}
...
export function attachSummaryEvidence(chat: STMessage[], floor: number, evidence: Awaited<ReturnType<typeof captureSummaryEvidence>>, coverageIndices: number[]) {
    if (!evidence)
        return;
    const leaf = getLeaf(chat[floor]);
    if (!leaf)
        return;
    chat[floor].extra![EVIDENCE_KEY] = { leaf: leaf.id, text: leaf.text, inputRefs: evidence.inputRefs,
        dependencies: evidence.dependencies, generatedContent: chat[floor].mes, basis: evidence.view.snapshot.id,
        coverage: coverageIndices.length === 2 && coverageIndices[1] === coverageIndices[0] + 1 && chat[coverageIndices[0]].is_user && !chat[coverageIndices[1]].is_user
...
export function assertSummaryEvidence(evidence: Awaited<ReturnType<typeof captureSummaryEvidence>>) {
    if (evidence)
        check(evidence.generation === dailyState.generation && evidence.host === hostVersion(), '摘要请求期间正文已改变，旧结果不能应用');
}
export async function confirmFork(parentId: string, prefixLength: number) {
    ...
    for (let i = 0; i < prefixLength; i++) {
        const host = ctx.chat[i], ref = parent.refs[i], source = parent.sources.get(ref.revision)!;
        check(mappings.some(m => m.message === ref.message && m.hostKey === host.extra?.[MESSAGE_KEY]) && source.content === host.mes, '分叉前缀身份或正文不匹配，请选择准确的父快照前缀');
    }
    const scope = hostScope();
