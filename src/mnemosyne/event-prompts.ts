...
仅输出 JSON：{"title":"标题","status":"open|resolved|dormant","keywords":["关键词"],"overview":"整条事件的精炼当前概要","progress":"仅本次新增内容的一段进展","latestProgress":{"memory":"实际最近进展的来源摘要ID","text":"${EVENT_PROGRESS_MAX_CHARS}字以内的最新进展小结"}}。
latestProgress 必须为 null 或含 memory、text 的对象；memory 只能选本次材料中的有效摘要ID。时间由程序取该来源的剧情时间。
overview 非空且必须不超过${EVENT_OVERVIEW_MAX_CHARS}字（含标点、数字、字母）；latestProgress.text 上限为${EVENT_PROGRESS_MAX_CHARS}字，替代旧模板中的30字限制，程序单独附加的剧情时间不计入；progress 无新增进展时为空字符串。所有字段必须保留，不输出 JSON 以外的文字。`;
