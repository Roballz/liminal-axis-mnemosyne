...
const LEGACY_INJECT_KEY = 'mnemosyne_memory';
/** 拆分后的 setExtensionPrompt keys;同 key 重复 set 即覆盖,天然幂等 */
const HISTORY_INJECT_KEY = 'mnemosyne_memory_history';
const STATE_INJECT_KEY = 'mnemosyne_memory_state';
/** 时间标签固定提示词槽:注入主对话,要求每条正文前后输出 <bbs_start>/<bbs_end> */
const TIMETAG_INJECT_KEY = 'mnemosyne_time_tag';
...
function buildView(
  ...
  if (chat) {
    for (let i = 0; i < chat.length; i++) {
      ...
      if (!summaryAllowed(leaf.id)) continue;
      byId.set(leaf.id, {
        id: leaf.id,
        kind: 'leaf',
        level: 0,
  ...
  for (const s of summaries) {
    if (!summaryAllowed(s.id)) continue;
    byId.set(s.id, {
      id: s.id,
      kind: 'comp',
      level: s.level,
  ...
  for (const s of byId.values()) for (const c of s.childIds ?? []) referenced.add(c);
