...
export function invalidateSummaryAncestors(childId: string): number {
  if (dailyInstalled()) return 0; // Canonical dependency validity retains history and marks needs_rebuild.
  if (!childId) return 0;
...
export function pruneBrokenComps(): boolean {
  if (dailyInstalled()) return false; // Missing children block recall; never erase historical summaries.
