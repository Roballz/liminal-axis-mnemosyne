...
| T-01 ↺ | 做最小 TT Adapter 探针 | 实现与真机证据已完成；Chat review 发现 request gate 并发竞态，返修后再关卡 |
...
无设计级阻塞。T-01 已完成主体实现和真机验证，但 Chat review 发现 `createRequestGate.begin()` 在并发异步 snapshot 下存在旧请求重新覆盖新请求的竞态；这是当前唯一收尾阻塞。

先返修 T-01 的 gate 与对应并发单测，再由 Chat 复核是否升为 `verified`。在 T-01 关卡前不创建正式 T-02 任务卡。

工程流程要求：Codex 只实现当前 task，并在收尾报告下一任务依赖／建议验证点；不得自动创建 T-02。

## 7. 需要用户提供

T-01 当前没有新的必需资料。TT 安装、柏宝书扩展目录、测试聊天和模型配置均已记录；不得重复询问。
