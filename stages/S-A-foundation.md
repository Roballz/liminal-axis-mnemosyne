...
| T-02 ▶ | 身份、历史版本与上下文的最小可执行契约 | planned；`tasks/T-02-identity-version-context.md`，校验/参考模型/正反例通过后 review |
...
- **T-02 落实。** schema/类型、跨对象校验、内存参考模型、prepare/幂等/有效性正反例；不提前实现生产导入器、LLM 重建器或数据库。旧 docs/examples 的临时字段与 Leaf 用语由该任务对齐。
...
文件优先级：用户最新明确确认与本节/决策记录 → 当前 planned task → 08 具体设计 → 06 正式契约（T-02 将更新）→ 07 首轮候选 → 早期架构/roadmap 示意。旧建议不能覆盖新确认；冲突需报告，不静默发明产品决定。
