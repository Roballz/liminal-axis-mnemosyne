...
## 2026-09-22 T05-R1页面状态返修

- 当前/历史快照选择前清除旧结果、详情及续页状态，保留UI代次与迟到结果保护。
- 新增真实产品按钮H1搜索/详情→H2切换、迟到详情及选择失败回归；新反例1/1，工作台集合10/10通过。
- 仅页面与必要测试，无存储改动、全量或原生重跑；保持implemented_unverified，交Chat回审。


## 2026-09-22 T-05手动搜索候选

- 新增固定snapshot/checkpoint有界正文搜索、Unicode完整折叠、来源与旧摘要对照、最小工作台和16MiB有界完整库导出。
- 复用现有owner和分页格式，查询不执行业务恢复；宿主定位暂明确回退档案。
- 一次全量355/355；实际TT沿既有合成库只读演示通过，业务写入0、完整包导出、共享owner及最终close核验成功。初次ESM缓存失败与入口清理修正均保留。
- 集中有限验证记录见T-05第8节，状态implemented_unverified；S-B待Chat验收，不创建/执行T-06。

## 2026-09-22 T-04原会话补证完成

- 先查原库确认正文25/25、摘要0/2且pending为空；用户授权后沿原会话只补2份摘要，未重导正文或替换旧请求。
- 实际TT/柏宝书来源与已发布正文、候选空位及摘要数据逐项相等；原Head/源不变、pending为空、关闭确认成功。续传及核验43.343秒，一次prepare/execute。
- 不追加全量/规模测试，完整证据交Chat复审；T-04保持implemented_unverified，不进入T-05。
## 2026-09-21 T-04 R1/R3/R2返修候选

- 续聊区分新段/副本/重叠；分叉区分父截取与已有子聊天，保留继承历史及子线尾部。
- 面板共用事件控制器核对稳定生成范围及旧资料白名单；稳定身份与locator分离，改名确认、v1绑定别名兼容。
- 原339项保留，新增6组业务入口/事件反例，最终候选一次345/345通过。真实插件补证待前端配合，implemented_unverified，T-05不启动。

## 2026-09-21 T-04 只读桥接

- TT原文和柏宝书API v1白名单投影、明确来源声明的旧资料；可恢复批次、正式绑定及逐楼映射，受控局部对账、持久暂停/人工确认、最小导入面板。
- 原分页协议内一起发布正文和桥接回执；完整分页包可空库恢复，旧逻辑v2小包不静默遗漏新增材料。无模型调用/注入/源端写回。
- 最终候选一次全量339/339。受控原生恢复核对通过，implemented_unverified，不进入T-05。


## 2026-09-21 T-03 P4-R1/R2与P5-R1返修

- 候选过滤/search/rebuild补版本漂移复核；备份诊断与导出流固定同一队列快照，校验目标checkpoint且允许源后续提交和不同目标library，关闭错误不覆盖核验错误。
- 物理目录有界增量AVL合并，最终1024条引用批次、16,384候选页/16MiB硬上限；保留不可变页、碰撞校验和完整发布持久化顺序，增加聚合计量及目录IO故障点。
- 333项回归保留原322项，旧包双向恢复及最终批次固定TT发布前/后强杀恢复通过。原1x资源结果、待审边界见notes/t-03-p4-repair-handoff.md及evals/t03/p4-repair/。状态implemented_unverified，自动维护与手机门禁保留，无T-04。

## 2026-09-21 T-03 P4索引/诊断增量与资源阻塞
...
T-04原生补证后续：保留TT的null候选槽及索引，1项针对性导入/恢复断言通过；真实25条/2摘要预览成功，原生导入超时后暂停，落盘确认pending。未重跑全量。

2026-09-22 T-04原会话核对：只查询原库，确认正文25/25、摘要0/2、partial/assets、pending空及明确关闭成功；不清库、不续传、不重跑回归。T-04保持implemented_unverified。
