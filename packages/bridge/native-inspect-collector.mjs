...
const files=new Map();
const run=process.argv.includes('--resume')?'20260922resume':'20260922inspect';
...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(value.run!==run||!['done','error'].includes(value.type))throw Error('Receipt');
    writeFileSync(`evals/t04/native-${run}.json`,JSON.stringify(value,null,2)+'\n');
    res.end('ok');console.log(JSON.stringify(value));server.close();
  }catch{res.writeHead(400).end();}
});
server.listen(19375,'127.0.0.1',()=>console.log(`Original-session module host ready: ${run}; user action required.`));
