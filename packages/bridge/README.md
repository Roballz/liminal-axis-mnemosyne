...
返修反例见 `tests/review.test.mjs`，经过实际读取适配、Importer和面板共用事件控制器。`native-live-entry.js` 仅在用户点击后运行真实公开接口与面板确认，限1–16条已有摘要的小样本，4分钟停止新操作；只保存脱敏计数/相等断言，不保存正文或摘要。该演示未收到成功回执前均为 pending，不能拿旧 synthetic 演示替代。
