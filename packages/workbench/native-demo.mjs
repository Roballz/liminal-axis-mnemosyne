...
export async function runNative(host=globalThis) {
  const run='20260922t05',namespace='mnemo-t03-paged-t05-20260922a';
  ...
  const stage=name=>{budget();out.stages.push({name,ms:Date.now()-start});};
  try {
    stage('create-once');
    const api=host.__TAURITAVERN__.api.db;
    // Exact empty-target assertion. No reuse, deletion, retry or namespace replacement.
    owner=await openPagedTestStore(api,namespace,{create:true});
    ...
    stage('import-preview');const plan=await importer.preview(input);
    stage('import-confirm');const bound=await importer.confirm(plan);
    ...
    out.type='done';
  } catch(e) {out.code=e.code??'ERROR';out.phase=out.stages.at(-1)?.name;}
  finally {
    panel?.close();
