...
const server=createServer(async(req,res)=>{
  ...
  const value=JSON.parse(body);if(value.run!=='20260922t05inspect'){res.writeHead(400).end();return;}
  writeFileSync('evals/t05/native-inspect-20260922.json',JSON.stringify(value,null,2)+'\n');res.end('ok');console.log(JSON.stringify(value));server.close();
});
