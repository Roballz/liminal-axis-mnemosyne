...
const server=createServer(async(req,res)=>{
  req.url=req.url.replace('/t05-8650107','');
  ...
  const value=JSON.parse(body);if(value.run!=='20260922t05read'){res.writeHead(400).end();return;}
  writeFileSync('evals/t05/native-read-20260922.json',JSON.stringify(value,null,2)+'\n');res.end('ok');console.log(JSON.stringify(value));server.close();
});
