...
最新用户范围覆盖下文先前资源完成线：当前只判断接口能力、正确性及持久化/恢复能否实现，不把旧电脑延迟作为可行性否决条件，不外推手机或新电脑性能。性能重测不再作为继续实施的前置条件；已发生的时间停止保留，原生完整1x未运行环节仍未验证，自动维护能力门禁保留。Chat按此调整后的范围复核，不自行升verified。
