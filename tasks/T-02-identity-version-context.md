...
状态：verified（2026-09-19 Chat 最终复核通过；范围限最小契约与 Node 内存参考模型）
...
| A05 | 切回旧候选可复用记忆；恢复旧清单产生新 snapshot，不复活旧 run |
...
## Chat 最终 review（2026-09-19）

**通过，T-02 升为 verified。** 审核实现提交 `1891043f6fc907236e12bb85d63ea823f43abca8`；R1～R5 在当前最小契约范围内关闭，完整记录见 `notes/t-02-final-review.md`。前述 implemented_unverified 均为当时实施回报的历史状态。

- Chat 在隔离 Linux / Node v22.16.0 实际复跑全部契约测试：48/48 通过；全部 11 个 .mjs 语法检查通过。执行前校验 13 个源码/测试/样例 Git blob SHA，与固定提交一致。
- 同一组 R5 新测试在恢复为实际旧 blob 的修复前副本上为 0/6 通过、6/6 失败；在本次实现上全通过，确认反例覆盖有效。
- 一跳/多跳及中间版本的矛盾导入拒绝；旧摘要无论列入 required 或仅出现在响应块，都不能注入；合法检查点、非当前历史纠错终点与子线旧版本仍可用。
- 探针18/18、完整仓库diff检查与Windows合计66/66为Codex回报，本轮Chat未独立复跑探针或完整diff检查。提交比较确认探针和T-03卡未变。
- 接受06 v0.3、对象schema_version=1、逻辑包v2及Node参考模型作为T-03的契约基线；不等同生产存储/崩溃恢复、手机联调、真实LLM派生或大规模性能已通过。
- 未决产品边界继续按第5节保留；T-03仍为proposal，下一步由Chat/用户核对和明确启用，本轮不修改、不执行该预备卡。
