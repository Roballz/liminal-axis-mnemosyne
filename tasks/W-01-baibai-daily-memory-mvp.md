...
用户明确批准正文／摘要／事件编辑后的保留、手改与更新工作流，并追加自定义表可设每批楼数、多表共用范围和一次请求、所有返回都进入手改窗口并在确认时二次校验原子写入。已授权范围继续交付到原安装分支。

云端断线后仅恢复旧目录，已从远端 91435fa 重建本轮修改并将阶段源码存至 work/edit-review-recovery 的 91c105d。用户随后要求精简测试尽快上传：限受影响测试、类型检查和构建，本轮不重跑全量、浏览器或真实手机；既有真实数据恢复证明不冒充本环境重跑。
...
## 2026-09-24 首次即误判为系统的历史恢复

用户反馈上轮后事件仍空、待审核108→118、首差异变为#507原系统当前AI（隐藏0～540，正文从541开始）。确认上轮仅覆盖先有正确AI版本再被误判的情况。本轮增加只读候选预览与用户确认：当前仍隐藏的普通AI回复，与本分支历史系统来源同身份、同完整正文、同宿主键及swipe时可确认具体版本配对。摘要、coverage、事件历史与处理回执统一使用分支范围配对；不重写旧记录，不自动批量compatible。新可导出字段sourceRoleRepairSchema=1，逻辑包v5兼容v1～v4，物理库v1不变。验收含首次误判、显式输入/高层注入、成员进展及回执、父分支隔离、导出恢复、取消/陈旧守卫、真实编辑/同文新身份/顺序/截止及坏包拒绝。状态implemented_unverified，手机待用户验证。

同轮用户追加检查所有聊天/分支隔离，依据是未迁移旧聊天打开即已归档。代码与新增桥接测试确认无绑定聊天自动新建独立故事/分支，复制父绑定的新子聊天仍冲突暂停；相同正文/宿主键不合并，当前召回候选与事件不跨聊天。修正切换等待时残留旧“已归档”字样，新增只读聊天名/分支档案ID辅助手机比对；未改变自动归档或继承语义。用户是否实际看到别处事件/内容仍待现场证据，不将合成通过宣称手机隔离已验证。


## 2026-09-24 改名绑定与误选分支恢复

用户续报已归档聊天改名后绑定冲突，重新选择继承后事件与待审核皆空，并询问旧核心包是否可救。复现原因是scope包含文件名，而继承入口新建分支不复制事件；原事件没有被此流程删除。本轮新增独立的只读查找/逐条身份正文预览/确认接回入口，继续原分支，误建绑定标为detached保留全部档案及映射。扩展尚未发布的逻辑包v5，Binding的rebindSchema=1/detached可导出并严格校验；活动scope唯一，物理库仍v1。恢复旧包后绑定缺失时暂停并引导接回，避免自动建空档案。验收覆盖直接改名、误选继承、原事件ID/概要/摘要、备份恢复、取消/事务回滚、宿主保存失败重试、真实子分支隔离、异角色/正文/身份/短聊天拒绝和坏包。状态implemented_unverified；远端发布仍待解除上一轮上传拒绝，不默认重新尝试。

## 2026-09-24 上传授权恢复与分叉差异补充

用户明确重试上传并点击允许，GitHub上传已获成功响应，前节的发布阻塞解除。追加父子档案均能继承父档案、不能继承子档案的反馈：确认分叉校验不比较角色，增强首差异楼层、身份/正文类别与匹配条数；不改变身份或分叉语义。新增综合反例复现两分支分别因旧角色待审、共享父前缀能继承而同文不同身份的子后缀拒绝，原事件与另一分支Head保持不变。全量399测试通过；手机原因仍未验证，恢复原事件应使用接回预览而非缩短前缀反复建分支。最终发布见最新回执。

同轮用户补充完整子→父→子→父顺序及AI/system双向来源差异；新增准确往返验证后bridge40/40通过，两分支分别确认旧角色修复后继续往返不丢事件、不重新待审。已发布实现e00d360至原分支，源码与dist树哈希一致；未操作用户手机或私人库，现场仍implemented_unverified。
