...
按用户反馈，楼层事件返回改为可编辑审阅后确认入库：有效/无效/空正文均显示，保留原始正文对照，明确格式与字段问题，格式正确且来源仍有效才发布。覆盖新建和立即更新；本次新增关联同事务提交，取消不留下半条更新。自动/批量任务保持原流程；无新持久字段。有限验收含坏值/截断/空串、修正无需重调API、取消零写入、重复确认拒绝、聊天改变后拒绝、流式末行及手机宽度弹窗。状态仅 implemented_unverified，实际结果见最终回执。


同轮追加长按菜单“重写概要”，在归档操作下方展示；输入要求、确认API、可编辑审阅后保存，覆盖已归档及已全部整理的链。复用现有材料/预算/版本保护和确认窗口，不新建链、不解除归档、不为纯改写追加进展。追加受影响集和浏览器路径验证后重跑最终全量及构建，前一轮365测试结果保留为追加需求前检查点，不冒充最终结果。

## 2026-09-24 加入事件的按钮布局与提示

按用户手机截图，将“立即更新 / 仅入库”改为等宽双列并留12px间距，主次配色及取消暗色保留；加入成功统一提示“已加入事件链”。核对重写概要仍发送自定义事件概要提示词和本次要求，并补全使用说明。不改关联、请求或存储逻辑；状态 implemented_unverified，结果见最终回执。

## 2026-09-24 父子聊天切换与待审核排查

用户报告同正文父、子分支切换后事件空白、刷新无效并出现108条待审核，随后确认导出核心包仍有整理好的事件。修复事件页只清空而不重载的问题，加入跨聊天读取守卫及已保存但被来源校验过滤的数量提示；不合并父子身份、不放宽召回过滤。待审核列表改为读取已归档视图，新增按需只读来源差异；切聊天清掉旧审核操作，人工保留增加事务内宿主守卫。无新持久格式、迁移、自动兼容或重新生成。合成父子切换与108条前缀失效可复现，真实手机原因仍需来源差异证据；状态 implemented_unverified，实际验证与推送见最终回执。

## 2026-09-24 隐藏楼层的来源角色修正

用户续报当前分支保存15条事件、15条均被校验隐藏；只读差异为#527原AI、现系统，并发现高层摘要仍存但历史注入只使用L0。沿用W-01授权，修正归档适配将无bbs_hidden标记的隐藏AI楼误判为system的问题，与既有摘要引擎区分原生typed系统楼的规则一致；宿主缓存/在途守卫使用同一来源角色。正常同步复用同一消息匹配的原有来源版本，不批量兼容、不重写摘要/事件、不删除误判历史。新增合成反例覆盖隐藏/取消隐藏与重载、旧误判恢复子分支事件及L2注入、父分支不变、真实系统/正文变化继续拦截。未访问用户私人聊天或手机，现场仍待更新复查；无新格式/迁移，状态implemented_unverified。
