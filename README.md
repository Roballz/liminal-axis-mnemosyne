...
8. 在“自定义表”建表、加字段、改行、搜索和翻页。允许 AI 后选择摘要范围手动填充；`manual` 字段不可被 AI 写，`append` 追加文本，`replace` 覆盖。
...
核心包包含：story/branch、正文及摘要版本、快照/映射、审核、事件及回执、自定义表与行、非秘密事件设置。只恢复到新空库，失败不替换原活动库。API 密钥不进入包。
...
- 自定义表自动周期填充不在本轮实现。删表/字段/行需要确认，逻辑隐藏并保留数据；不提供自动清理。
- 当前库接口针对日常单聊天规模；大型整库导出/导入与超大目录的手机内存、延迟、系统回收仍待实测。
- 合成浏览器测试入口已提供，但当前环境缺 Chromium，下载内容无效，未完成渲染测试；TT 桌面/手机均未运行。
