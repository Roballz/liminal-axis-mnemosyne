> 2026-09-24：事件链改为楼层卡片手动归组，AI 只写概要；表列表新增按正文范围批量补表。档案分支操作统一为“继承档案”。状态仍为 implemented_unverified。详见 [使用说明](docs/daily-memory-user-guide.md)。
...
6. 在聊天楼层卡片点事件图标，按自己的事件边界新建或加入链。仅入库不调用模型；收束后选择更新概要，或到事件页“批量整理”。自动更新默认关闭，可按消息楼间隔启用。
7. 展开事件卡看完整概要和成员摘要；在编辑弹窗修改标题、状态、概要、关键词，删除需再次确认。召回保留原事件进展节选规则，待实测后再调整。
8. 在“自定义表”建表、加字段、改行、搜索和翻页。表列表 → 设计表设置字段说明和 AI 提示词；新摘要同一次模型调用增量填表。支持覆写、追加、首次写入后锁定，保留旧 manual 字段限制；隐藏行不发送。关闭自动填表后可在表列表用“批量补表”，多选表和正文楼层范围。
...
- 复制聊天携带另一绑定时暂停自动归档，要求选择独立新故事或“继承档案”（固定前缀分叉）。分叉先校验消息身份与正文；不存在可靠映射时不猜测。
- 召回继续使用 Query、多 query max、BM25、RRF、rerank 和原配额。后端返回的正文副本不能覆盖 canonical 正文。rerank 未执行/失败时分数为 null；回退选档仍可按原余弦阈值执行。

## 迁移包边界与回退

新版核心包格式 v4，可恢复旧 v1/v2/v3；物理 IndexedDB 不升级。核心包包含：story/branch、正文及摘要版本、快照/映射、审核、事件及回执、事件归档标记、自定义表与行、非秘密事件设置。只恢复到新空库，失败不替换原活动库。API 密钥和宿主自定义提示词不进入包。
...
- 概要更新发送所选链全部合法成员，超预算暂停；不由 AI 自动归链。预算使用字符上限，不冒充精确 tokenizer。事件扩展有独立字符预算，属于额外上下文开销。
